...
unitTests =
  [ testCase "parse integers success" $
      parse parseExpr "" "123" @?= Right (Const (mkImmNativeType U64) (NativeInt 123)),
    --
    ...
    testCase "parses let stmts with const" $
      parse parseLet "" "x : u64 = 3;" @?= Right (Let (mkImmNativeType U64) "x" (Const (mkImmNativeType U64) (NativeInt 3))),
    --
    testCase "parses let stmts with mut" $
      parse parseLet "" "x: mut u64 = 3;" @?= Right (Let (mkMutNativeType U64) "x" (Const (mkImmNativeType U64) (NativeInt 3))),
    --
      ...
      parse parseLet "" "x: u64 = 1 + y;"
        @?= Right
          ( Let
              (mkImmNativeType U64)
              "x"
              (BinOp TyToInfer Add (Const (mkImmNativeType U64) (NativeInt 1)) (Var TyToInfer "y"))
          ),
    --
