  ...
  testGroup
    ...
    [ testCase "parse integers success" $
      ...
      testCase "parses let stmts with const" $
        parse parseLet "" "x: i64 = 3;" @?= Right (Let (Native I64) "x" (Const I64 3)),
      --
      testCase "parses let stmts with var" $
        parse parseLet "" "x:i64 = y;" @?= Right (Let (Native I64) "x" (Var TyToInfer "y")),
      --
      testCase "parses return stmt" $
        parse parseReturn "" "return y;" @?= Right (Return (Var TyToInfer "y")),
      --
      testCase "parses let stmt with sum" $
        parse parseLet "" "x: i64 = 1 + y;" @?= Right (Let (Native I64) "x" (BinOp Add (Const I64 1) (Var TyToInfer "y"))),
      --
      ...
      testCase "parses let stmts 1" $
        parse parseLet "" "y:   i64 = -30;" @?= Right (Let (Native I64) "y" (UnaryOp Neg (Const I64 30))),
      --
