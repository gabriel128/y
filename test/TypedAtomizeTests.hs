...
module TypedAtomizeTests (test_typed_atomizer) where
...
runComplexStmts :: [TypedStmt] -> (Context, TypedProgram)
runComplexStmts = fromRight' . run
  where
    run :: [TypedStmt] -> Either Text (Context, TypedProgram)
    run stmts = runStateErrorEff defaultContext (removeComplexStmts (newTypedProgram stmts))

-- We only test valid programs
runTypeCheck :: TypedProgram -> Either Text (Context, TypedProgram)
runTypeCheck program =
  runStateErrorEff defaultContext $ typeCheck program

getTypeCheckedProg :: Text -> Either Text TypedProgram
...
unitTests =
  [
    testCase "remove complex ops do nothing when not needed" $ do
      ...
      let (Context locals 0, prog') = runComplexStmts (typedProgStmts prog)
    ...
    testCase "multiple stmts remove complex ops with Unary" $ do
      ...
      expectedProg <- liftEither $ getTypeCheckedProg
