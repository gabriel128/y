...
insertEdge (nodeIdA, nodeIdB) (Graph nodes) =
  let
    insertFn nodeId createdNode oldNode =
      Node nodeId (nodeEdges createdNode `Set.union` nodeEdges oldNode)
    nodes' = Map.insertWith (insertFn nodeIdA)
                            nodeIdA
                            (newNode nodeIdA [nodeIdB])
                            nodes
    nodes'' = Map.insertWith (insertFn nodeIdB)
                             nodeIdB
                             (newNode nodeIdB [nodeIdA])
                             nodes'
  in
    Graph nodes''
...
insertNode node@(Node a edges) (Graph nodes) =
  let insertEdgeToNode edge (Node val nodeEdges) =
        Node val (Set.insert edge nodeEdges)
      oldNodes = mapWithKey
        (\k node' -> if k `elem` edges then insertEdgeToNode a node' else node')
        nodes
...
dfs source (Graph nodes) =
  reverse . snd <$> go (Map.lookup source nodes) Set.empty []
 where
  ...
  reducer visited nodeId = do
    node <- maybeToRight (T.pack $ "Node " <> show nodeId <> " not found")
                         (Map.lookup nodeId nodes)
...
bfs source (Graph nodes) = doBfs (Map.lookup source nodes)
 where
  ...
  doBfs (Just (Node nodeId _)) =
    go Set.empty (Queue.enqueue nodeId Queue.new) 0
  ...
  go visited queue distance
    ...
    | otherwise = do
      ...
      currentNodes <- maybeToRight "Node not found"
                                   (mapM (`Map.lookup` nodes) currentNodeIds)
      let nodeIdsToEqueue = concatMap
            (Set.toList . flip Set.difference visited' . nodeEdges)
            currentNodes
...
type SaturationMap a = Map a (Set GraphColor)

newSaturationMap :: Set (Node a) -> SaturationMap a
newSaturationMap nodes = Map.empty

maxSaturated :: SaturationMap a -> a
maxSaturated = undefined

maxSaturatedNotVisited :: SaturationMap a -> Set (Node a) -> Node a
maxSaturatedNotVisited = undefined

colorsNotInAdj :: Set GraphColor -> Set a -> Set GraphColor
colorsNotInAdj = undefined

-- | Given a graph and a set of graph colours, it maps the 
colorGraph
  :: forall a . Ord a => Graph a -> Set GraphColor -> Map a GraphColor
colorGraph graph colors =
  let w = Set.fromList (Map.elems $ graphNodes graph)
  in  while w (newSaturationMap w) Set.empty Map.empty
  where while = undefined
    -- while [] _saturationMap _visited res = res
    -- while w saturationMap visited colorMapping =
    --   let u = maxSaturatedNotVisited saturationMap visited
    --       c = min (colorsNotInAdj colors (nodeEdges u))
    --    in while (Set.delete u w) saturationMap (Set.insert u visited) (Map.insert (nodeVal u) c colorMapping)
